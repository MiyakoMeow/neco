...
| 层级 | 安全机制 | 描述 | 实现状态 |
|------|----------|------|----------|
| L1 | WASM双计量沙箱 | 燃料计量+时代中断 | 规划中 |
| L2 | Merkle哈希链审计 | 加密链接，防篡改 | 未启动 |
| L3 | 信息流污点追踪 | 敏感信息追踪 | 未启动 |
| L4 | Ed25519签名清单 | 身份密码学验证 | 未启动 |
| L5 | SSRF防护 | 阻止内网IP、云元数据 | 已实现 |
...
| L13 | 循环守卫 | SHA256循环检测+断路器 | 未启动 |
| L14 | 会话修复 | 7阶段消息历史验证 | 未启动 |
| L15 | 路径穿越防护 | 规范化+symlink转义检测 | 已实现 |
| L16 | GCRA速率限制 | 成本感知令牌桶限流 | 未启动 |
...
| 优化策略 | 预期效果 | 实施难度 |
|----------|----------|----------|
| 移除未使用的 crate 依赖 | -2~5MB | 低 |
| 使用 `opt-level = "z"` 优化 | -1~3MB | 低 |
| 激进 LTO (`lto = "fat"`) | -2~4MB | 中 |
| 减少运行时特性 | -1~2MB | 中 |
| **合计（优化后）** | **~25-30MB** | - |
...
> **版本说明**：以下技术参考基于 ZeroClaw 架构设计，具体实现可能有所不同。

| 维度 | ZeroClaw | NeoCo 架构目标 | NeoCo 当前状态 |
|------|----------|----------------|----------------|
| **定位** | Rust 原生自主 AI 助手运行时 | 多智能体协作 AI 应用 | 设计阶段 |
| **架构** | Trait-driven + Factory | Trait-driven + 依赖反转 | 待实现 |
| **内存占用** | <5MB | <50MB | 空壳 |
| **启动速度** | <10ms | <100ms | 空壳 |
| **Provider** | 多 Provider 抽象 | 多 Provider 支持 | 待实现 |
| **安全** | OTP/E-Stop/配对/沙箱 | 16 层安全体系 | 待实现 |
| **通信** | IPC + Channel 抽象 | EventBus + 工具调用 | 待实现 |
| **Memory** | 向量+BM25混合搜索 | 混合搜索支持 | 设计阶段 |
