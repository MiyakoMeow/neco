...
- `activate::skill`：激活skill。
- `activate::mcp`：激活mcp。
- `fs::read`：读取文件
- `fs::list`/`fs::ls`：读取目录
- `fs::write`：完全重写
- `fs::edit`：编辑已有文件
- `fs::delete`/`fs::rm`：删除文件/目录
  - 删除目录时，要求目录为空。
- `mcp::xxx`：对应配置文件`mcp_servers.xxx`。
- `multi-agent::spawn`：生成下级Agent。
- `multi-agent::send`：向指定Agent传递消息。
- `multi-agent::report`：向用户报告。支持的类型：
  - `progress`：进度报告
  - `result`：结果报告
  - `question`：问题报告
- `context`：上下文观测工具。
  - `context::observe`：查看当前上下文的详细信息。
    ...
    - 格式化为结构化的表格或列表，便于阅读
    - 支持过滤参数（如只查看特定类型的消息）
  - `context::compact`：上下文压缩工具，用于压缩上下文以节省token。
- `workflow::pass`：工作流通过工具。
- `workflow::option`：工作流选项工具，用于向用户提供选项选择。
- `tui::question`：用于向用户提问。仅限TUI运行模式下的非no-ask模式可用。
- 如果有其它需要的工具，直接补充。
