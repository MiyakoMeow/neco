...
> **依赖说明**：本模块使用 [async-openai](https://crates.io/crates/async-openai) crate **版本 0.33.0** 进行 OpenAI API 调用。
...
> **TODO：实现故障转移功能**
> 
> 根据需求文档，ModelGroupClient 需要实现以下故障转移逻辑：
> - 当模型调用失败（重试 3 次全部失败）时，自动切换到下一个模型
> - 按配置顺序尝试模型，不循环
> - 所有模型都失败时返回聚合错误
